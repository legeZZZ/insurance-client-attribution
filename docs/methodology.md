...
区间覆盖率必须单独解读：Gaussian 为 0.8775，旧 Student-t plug-in-`tau` 路径仅为 0.3075；
联合传播 `mu/tau/nu` 后提升到 0.9475。四真值族 200 数据集的总体覆盖率为 0.9383，
最低单族为 0.9100。不过联合后验在仓库回放中的方向召回为 0、moderation RMSE 为 0.00398，
尚未通过效用门槛，因此仍不是生产默认。所有结果可一键复现。
...
- EconML / CausalML：异质效应（HTE）估计——我们的 `estimate_hte_nested` 做嵌套分组的 HTE。
  Student-t 随机效应目前只在 `estimate_hte` 中实现。联合离散后验会显式传播 `mu/tau/nu`
  不确定性，解决了 plug-in-`tau` 的严重欠覆盖；但效用门槛仍未通过，因此它继续作为实验路径，
  不替代生产默认的 Gaussian 路径。
