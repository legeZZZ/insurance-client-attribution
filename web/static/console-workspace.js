...
function enhanceLibrary(){const research=wsData().research||[];document.getElementById('view-library').insertAdjacentHTML('afterbegin',`<div class="card ws-library-extra"><h3>因子是有角色、有时点、有验证条件的调查线索</h3><div class="ws-grid"><div><div class="ws-number">${wsData().registry_count??D.factors.length}</div><p class="ws-muted">当前注册表记录 · 含研究候选</p></div><div><div class="ws-number">${research.length}</div><p class="ws-muted">附操作化定义的研究因子</p></div><div><div class="ws-number">${(wsData().sources||[]).length}</div><p class="ws-muted">研究来源条目 · 不等于本业务验证</p></div></div><div class="ws-boundary">方案目标为 191 个候选 / 9 个命名空间；这里按实际加载记录计数。水平 / 速度 / 加速度是派生序列，不重复当作独立业务因子。</div><div class="ws-actions"><button class="btn ghost" onclick="wsShowResearch()">检索角色、来源与验证陷阱 →</button></div></div>`);}
