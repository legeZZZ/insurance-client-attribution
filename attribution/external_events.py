...
PUBLIC_EVENT_TIMELINE: list[dict[str, Any]] = [
    ...
    {"event_id": "lpr_2024_07", "date": "2024-07-22",
     "kind": "macro_rate", "desc": "LPR 一年期/五年期各下调 10bp（公开发布）"},
    {"event_id": "reg_baoxing_heyi", "date": "2024-08-02",
     "kind": "regulation", "desc": "银保渠道费用规范持续执行（报行合一，公开政策）"},
    {"event_id": "school_season_2024_09", "date": "2024-09-01",
     "kind": "seasonality", "desc": "开学季（周期性公开日历事件）"},
]
...
def map_anomalies_to_events(
    ...
    return {
        "mapped": mapped,
        "unmapped": unmapped,
        "unused_events": [ev["event_id"] for ev in events if ev["event_id"] not in used],
        "coverage": {
            "detected_anomalies": total,
            "mapped": len(mapped),
            "mapping_coverage": round(len(mapped) / total, 4) if total else None,
            "timeline_utilization": round(len(used) / len(events), 4) if events else None,
...
def _simulate_panel(seed: int = 20260811) -> dict[str, Any]:
    ...
    truth["events"] = [{"event_id": "promo_618_2024", "day": promo_day},
    ...
    treated = control + gap
    return {"days": days.tolist(), "control": control.tolist(),
            "treated": treated.tolist(), "truth": truth}
...
def main() -> None:
    ...
    result = attribute_baseline(
        days=panel["days"], control=panel["control"], treated=panel["treated"],
        change_registry=[], external_registry=[], experiments={},
    ...
    out_path = Path(__file__).resolve().parent.parent / "outputs" / "external_event_mapping.json"
