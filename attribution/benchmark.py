...
def _segment_predictive_brier(rows, fields=("device_low_end", "channel"), pseudo_n=50.0):
    ...
    for r in rows:
        ...
        cells[key][1] += 1
    rate = {k: (v[0] + pseudo_n * pooled_rate[k[-1]]) / (v[1] + pseudo_n)
            for k, v in cells.items()}
    return sum((r["clicked"] - rate[tuple(r.get(f) for f in fields) + (r["treatment"],)]) ** 2
...
def _run_single_seed(seed: int, mismatched: bool) -> dict[str, Any]:
    ...
    p_hat = (treatment["clicks"] + control["clicks"]) / (treatment["impressions"] + control["impressions"])
    brier = sum((r["clicked"] - p_hat) ** 2 for r in estimation_rows) / len(estimation_rows)
    ...
    brier_floor = sum(r["_oracle_true_p"] * (1 - r["_oracle_true_p"]) for r in oracle_rows) / len(oracle_rows)
    ...
    rng_segs = [
        ("channel=organic", "organic"), ("channel=paid", "paid"), ("channel=social", "social"),
    ...
    hte = estimate_hte(segments, practical_threshold=0.01, moderation_threshold=0.005,
                       seed=seed, discovery=False)
    ...
    for seg in hte["segments"]:
        ...
        if seg_id in true_harm_segments:
            ...
            if shrunk_flag_p >= 0.95:
                shrunk_fp += 1
    moderation_rmse_raw = math.sqrt(sum(mod_err_raw) / len(mod_err_raw)) if mod_err_raw else 0.0
    moderation_rmse_shrunk = math.sqrt(sum(mod_err_shrunk) / len(mod_err_shrunk)) if mod_err_shrunk else 0.0
    ...
    for item in effects:
        ...
        if is_true_factor:
            total_true += 1
            if item["significant"] and (item["component_effect"] < 0) == (true_effect < 0):
                recovered += 1
...
def run_benchmark(seeds=(101, 211, 307, 401, 503), mismatched_seeds=(601, 701)) -> dict[str, Any]:
    ...
    def summarize(subset):
        return {
            ...
            "bundle_ate_rmse": round(math.sqrt(mean("_ate_sq", [
                {**r, "_ate_sq": r["bundle_ate_error"] ** 2} for r in subset]) or 0.0), 6),
            ...
            "raw_fp_segments_total": sum(r["raw_false_positive_segments"] for r in subset),
            "shrunk_fp_segments_total": sum(r["shrunk_false_positive_segments"] for r in subset),
