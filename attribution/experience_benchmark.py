...
SEEDS = [(101, False), (211, False), (307, False), (401, False), (503, False),
         (601, True), (701, True)]
...
def _run_period(seed: int, mismatched: bool, traffic: float,
    ...
    if store is not None:
        ...
        devs = [d for d in (
            store.arm_deviation("bundle.control", control["clicks"] / max(control["impressions"], 1),
                                max_total=PRIOR_PSEUDO_N),
            store.arm_deviation("bundle.treatment", treatment["clicks"] / max(treatment["impressions"], 1),
                                max_total=PRIOR_PSEUDO_N),
        ) if d is not None]
        if devs:
    ...
    bundle = bundle_compare(control, treatment, prior=prior, practical_threshold=0.005, seed=seed)
    ...
    hte = estimate_hte(segments, practical_threshold=0.01, moderation_threshold=0.005,
                       seed=seed, discovery=False, shrinkage_strength=nu)

    spurious = [s for s in hte["segments"] if s["segment_id"].startswith("hash_bucket")]
    mod_rmse = math.sqrt(sum(s["moderation_shrunk"] ** 2 for s in spurious) / len(spurious))
    ...
    record = {
        ...
        "prior_pseudo_impressions": (sum(prior[0]) + sum(prior[1])
                                     if isinstance(prior[0], (tuple, list)) else 2.0),
    }
    ...
    if store is not None:
        ...
        record["pid"] = pid
        store.update_predictions({s["segment_id"]: s["effect_shrunk"] for s in hte["segments"]})
        store.end_period()
    return record
...
def run_experience_ablation(store_path=None) -> dict[str, Any]:
    ...
    return {
        ...
        "adaptive_experience_store": {
            ...
            "mismatch_alarm": {
                "fired_periods": [r["period_seed"] for r in adaptive if r["mismatch_alarm"]],
                "true_mismatch_seeds": [s for s, m in SEEDS if m],
                "deviation_by_period": [
                    {"seed": r["period_seed"], "mismatched": r["mismatched"],
                     "deviation": (round(r["prior_deviation"], 5)
                                   if r["prior_deviation"] is not None else None)}
                    for r in adaptive],
                "note": "alarm semantics: fires at mismatch onset; the decayed "
                        "store then adapts, so later mismatched periods may not fire",
            },
            "nu_trajectory": [round(r["shrinkage_strength"], 1) for r in adaptive],
            "prior_pseudo_impressions": [round(r["prior_pseudo_impressions"], 1) for r in adaptive],
        },
...
def main() -> None:
    ...
    out = Path(__file__).resolve().parent.parent / "outputs" / "experience_ablation.json"
