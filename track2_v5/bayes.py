...
def _normal_cdf(value: float, mean: float, standard_error: float) -> float:
    scale = max(float(standard_error), 1e-9)
    z = (float(value) - float(mean)) / scale
    return 0.5 * math.erfc(-z / math.sqrt(2.0))


def _ridge_fit(matrix: np.ndarray, target: np.ndarray, alpha: float) -> np.ndarray:
...
def _condition_matches(row: Mapping[str, Any], condition: Mapping[str, Any]) -> bool:
    for field, expected in condition.items():
        ...
        if isinstance(expected, Mapping):
            actual_number = _as_finite_float(actual, field)
            if "min" in expected and actual_number < float(expected["min"]):
                return False
            if "max" in expected and actual_number > float(expected["max"]):
                return False
            if "gt" in expected and actual_number <= float(expected["gt"]):
                return False
            if "lt" in expected and actual_number >= float(expected["lt"]):
                return False
...
def _build_hte_terms(
    ...
    for field in feature_columns:
        if field in continuous:
            values = np.asarray(
                [
                    _as_finite_float(row.get(field, np.nan), field)
            ...
            fill = float(np.mean(values[finite])) if np.any(finite) else 0.0
            values = np.where(finite, values, fill)
            std = float(np.std(values))
            z = (values - float(np.mean(values))) / max(std, 1e-9)
            for name, vector, basis in (
                (field, z, "standardized"),
                (f"{field}^2", z**2 - float(np.mean(z**2)), "quadratic"),
                (f"{field}:high", np.maximum(z, 0.0), "positive_hinge"),
            ):
                term_values.append(np.asarray(vector, dtype=float))
                terms.append(
                    {
                        "term": name,
                ...
                diagnostics["basis_feature_count"] += 1
            continue

        values = [str(row.get(field, "<missing>")) for row in rows]
        levels = sorted(set(values))
        for level in levels:
            vector = np.asarray([1.0 if value == level else 0.0 for value in values])
            if float(np.std(vector)) < 1e-9:
                continue
            term_values.append(vector)
            terms.append(
                {
                    "term": f"{field}={level}",
                    "source_feature": field,
    ...
    if term_values:
        main_matrix = np.column_stack(term_values)
    else:
        main_matrix = np.zeros((len(rows), 0), dtype=float)

    interaction_values: list[np.ndarray] = []
    interaction_terms: list[dict[str, Any]] = []
    main_limit = min(main_matrix.shape[1], 18)
    for left in range(main_limit):
        for right in range(left + 1, main_limit):
            if len(interaction_values) >= max_interactions:
                break
            vector = main_matrix[:, left] * main_matrix[:, right]
            if float(np.std(vector)) < 1e-9:
                continue
            interaction_values.append(vector)
            interaction_terms.append(
                {
                    "term": f"{terms[left]['term']} × {terms[right]['term']}",
                    "source_feature": f"{terms[left]['source_feature']}+{terms[right]['source_feature']}",
        ...
        if len(interaction_values) >= max_interactions:
            break
    term_values.extend(interaction_values)
    terms.extend(interaction_terms)
    diagnostics["interaction_feature_count"] = len(interaction_values)

    if term_values:
        raw_matrix = np.column_stack(term_values).astype(float)
        means = np.mean(raw_matrix, axis=0)
        stds = np.std(raw_matrix, axis=0)
        matrix = (raw_matrix - means) / np.maximum(stds, 1e-9)
    else:
        matrix = np.zeros((len(rows), 0), dtype=float)
    return matrix, terms, diagnostics


def estimate_high_dimensional_hte(
    rows: Sequence[Mapping[str, Any]],
    treatment_column: str,
    outcome_column: str,
    feature_columns: Sequence[str],
    continuous_features: Sequence[str],
    subgroup_rules: Sequence[Mapping[str, Any]],
    ...
    """Cross-fitted high-dimensional HTE for randomized Bernoulli experiments.

    The aggregate decision can stay Beta-Binomial; this layer estimates CATE
    from row-level pre-treatment features and summarizes overlapping subgroups.
    """
    if len(rows) < 50:
    ...
    treatment = np.asarray(
        [_as_finite_float(row.get(treatment_column), treatment_column) for row in rows],
        dtype=float,
    )
    outcome = np.asarray(
        [_as_finite_float(row.get(outcome_column), outcome_column) for row in rows],
        dtype=float,
    )
    ...
    observed_propensity = float(np.mean(treatment))
    p = observed_propensity if propensity is None else float(propensity)
    if not math.isfinite(p) or p <= 0.02 or p >= 0.98:
        raise ValueError("propensity must be in (0.02, 0.98)")
    ...
    matrix, terms, term_diagnostics = _build_hte_terms(
        rows, feature_columns, continuous_features, max_interactions
    )
    design = np.column_stack([np.ones(len(rows)), matrix])
    rng = np.random.default_rng(seed)
    order = rng.permutation(len(rows))
    fold_ids = np.zeros(len(rows), dtype=int)
    fold_ids[order] = np.arange(len(rows)) % folds
    predictions = np.zeros(len(rows), dtype=float)
    fold_summaries: list[dict[str, Any]] = []
    for fold in range(folds):
        train = fold_ids != fold
        test = ~train
        coef = _ridge_fit(design[train], transformed_outcome[train], ridge_alpha)
    ...
    predictions = pooled_effect + (predictions - float(np.mean(predictions)))
    cate = np.clip(predictions, -0.25, 0.25)
    full_coef = _ridge_fit(design, transformed_outcome, ridge_alpha)
    residual = transformed_outcome - design @ full_coef
    residual_se = float(np.std(residual))

    top_moderators = []
    for term, coef in zip(terms, full_coef[1:]):
        ...
        top_moderators.append(
            {
                **term,
                ...
                "direction": "amplifies_harm" if coef < 0 else "reduces_harm",
            }
        )
    top_moderators.sort(key=lambda item: item["importance"], reverse=True)

    membership_masks: list[np.ndarray] = []
    subgroup_summaries: list[dict[str, Any]] = []
    for rule in subgroup_rules:
        condition = rule.get("condition") or rule.get("where") or {}
        if not isinstance(condition, Mapping):
            raise ValueError("subgroup rule condition must be a mapping")
        mask = np.asarray([_condition_matches(row, condition) for row in rows])
        membership_masks.append(mask)
    membership_count = (
        np.sum(np.column_stack(membership_masks), axis=1)
        if membership_masks
        else np.zeros(len(rows), dtype=float)
    )
    total_memberships = int(np.sum(membership_count))
    rows_in_any_subgroup = int(np.sum(membership_count > 0))
    rows_with_overlap = int(np.sum(membership_count > 1))
    prior_count = 800.0
    ...
    return {
        ...
        "aggregate_model_dependency": "beta_binomial_bundle_gate",
        "evidence_level": "EXPLORATORY_HDIM_HTE",
        "identification": "randomized_assignment_required; subgroup selection remains exploratory unless pre-registered",
        "effect_scale": "probability_difference",
        "transformed_outcome": "Y * (T - p) / (p * (1-p))",
        ...
        "diagnostics": {
            **term_diagnostics,
            ...
            "overlapping_subgroup_count": len(subgroup_rules),
            "total_subgroup_memberships": total_memberships,
            "rows_in_any_subgroup": rows_in_any_subgroup,
            "rows_with_multiple_subgroups": rows_with_overlap,
            "residual_standard_error": residual_se,
        },
        "folds": fold_summaries,
        "top_moderators": top_moderators[:12],
        "subgroups": subgroup_summaries,
        "individual_cate_preview": [
            {
                "row": int(index),
                "cate": float(cate[index]),
                "memberships": int(membership_count[index]),
            }
            for index in np.argsort(cate)[:10]
        ],
    }
