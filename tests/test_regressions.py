...
class RegressionTests(unittest.TestCase):
    ...
    def test_high_dimensional_hte_models_overlap_and_continuous_features(self) -> None:
        rows_all, _ = generate_bundle_stage(seed=9, n=4_000)
        rows = sanitize(rows_all)
        result = estimate_high_dimensional_hte(
            rows,
            ...
            subgroup_rules=(
                ...
                {
                    "subgroup_id": "paid_pressure",
                    "label": "付费高压力",
                    "condition": {
                        "channel": "paid",
                        "market_pressure_index": {"min": 0.45},
                    },
                },
                {
                    "subgroup_id": "complex_fast_competitor",
                    "label": "复杂报价 × 竞品快",
                    "condition": {
                        "quote_form_step_count": {"min": 6},
                        "competitor_quote_speed_index": {"min": 0.40},
                    },
                },
            ),
            folds=4,
        ...
        self.assertEqual(result["model"], "crossfit_overlap_ridge_cate")
        self.assertEqual(result["crossfit_folds"], 4)
        self.assertGreater(result["diagnostics"]["continuous_feature_count"], 0)
        self.assertGreater(result["diagnostics"]["interaction_feature_count"], 0)
        self.assertGreater(
            result["diagnostics"]["total_subgroup_memberships"],
            result["diagnostics"]["rows_in_any_subgroup"],
        )
        self.assertGreater(result["diagnostics"]["rows_with_multiple_subgroups"], 0)
        self.assertTrue(result["top_moderators"])
        self.assertTrue(result["individual_cate_preview"])
    ...
    def test_confirmation_requires_a_complete_normalized_command(self) -> None:
        ...
        self.assertEqual(handle_message(session_id, "line_a")["stage"], "confirm")
        with patch(
            "track2_v5.agent_chat.run_scenario",
    ...
    def test_console_uses_current_api_contract_for_all_scenarios(self) -> None:
        ...
        self.assertIn("/api/track2/chat", html)
        self.assertIn("/api/track2/scenario-run", html)
        self.assertNotIn("/api/attribution/", html)
        ...
        self.assertIn("三维证据关系图", html)
        self.assertIn("window.setNetworkPhase", html)
        self.assertIn("cc-network-tooltip", html)
        self.assertIn("window.updateEvidenceGraph", html)
        self.assertIn("候选因子库 · 关系可视化", html)
        self.assertIn("时间/范围关联", html)
        self.assertIn("cc-network-node-labels", html)
        self.assertIn("cc-library-tooltip", html)
        self.assertIn("内部因子 → 异常范围 → 定向 A/B", html)
        self.assertIn("外部信号 → 异常范围 → 准实验", html)
        self.assertIn("未知候选 → 异常范围 → 补数与复核", html)
        self.assertIn("报价 API 超时率", html)
        self.assertIn("保险搜索热度", html)
        self.assertIn("埋点缺口率", html)
        self.assertIn("一次求导", html)
        self.assertIn("二次求导", html)
        self.assertIn("结算·一阶", html)
        self.assertIn("报价超时·一阶", html)
        self.assertIn("汇率·一阶", html)
        self.assertIn("竞价压力·二阶", html)
        self.assertIn("派生视图参与搜索，但不是新的父因子", html)
        self.assertNotIn("raw_layer", html)
        self.assertNotIn("velocity_layer", html)
        self.assertNotIn("acceleration_layer", html)
        self.assertIn("组合假设", html)
        self.assertIn("验证后才升因果", html)
        self.assertIn("验证后因果", html)
        self.assertIn("highlightEdges", html)
        self.assertIn("glow.visible", html)
        self.assertIn("cross-fit overlap ridge CATE", html)
        self.assertIn("高维 HTE / 重叠 subgroup", html)
        self.assertIn("重叠样本", html)
        self.assertIn("连续特征 CATE", html)
        self.assertIn("报价表单步骤数", html)
        self.assertIn("聚合平台排名下滑", html)
        self.assertIn("UTM 归因空值率", html)
        self.assertIn("factorId.startsWith('external.')", html)
        self.assertNotIn("HIGH PRIORITY · B-LINE", html)
    ...
    def test_line_a_exposes_high_dimensional_overlap_hte(self) -> None:
        report = run_scenario("line_a")
        metrics = report["metrics"]
        hte = report["key_outputs"]["high_dimensional_hte"]
        self.assertEqual(metrics["hte_model"], "crossfit_overlap_ridge_cate")
        self.assertGreaterEqual(metrics["hte_continuous_features"], 10)
        self.assertGreaterEqual(metrics["overlap_subgroups"], 5)
        self.assertGreater(hte["diagnostics"]["rows_with_multiple_subgroups"], 0)
        self.assertIn("top_hte_moderator", metrics)

    def test_line_b_fixture_has_realistic_factor_inventory(self) -> None:
        report = run_scenario("line_b")
        association = report["key_outputs"]["association_discovery"]
        library = report["key_outputs"]["factor_library"]
        factors = {
            candidate.get("parent_factor_id", candidate["factor_id"])
            for candidate in association["candidates"]
        }
        self.assertGreaterEqual(association["candidate_count"], 100)
        self.assertGreaterEqual(library["candidate_count"], 35)
        self.assertGreaterEqual(association["search_manifest"]["M"], 35)
        self.assertIn("internal.quote_api_timeout_rate", factors)
        self.assertIn("internal.quote_form_step_count", factors)
        self.assertIn("external.search_trend_insurance", factors)
        self.assertIn("external.aggregator_rank_drop_index", factors)
        self.assertTrue(any(factor.startswith("quality.") for factor in factors))
    ...
    def test_experience_scenario_uses_current_trajectory_schema(self) -> None:
        ...
        with (
            ...
            patch(
                "track2_v5.experience_benchmark.run_experience_ablation",
