"""Process-isolated hidden-seed benchmark for causal governance."""
...
def _run_isolated_worker(public_datasets: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    if completed.returncode != 0:
        raise RuntimeError("isolated benchmark worker failed: " + completed.stderr.strip())
    payload = json.loads(completed.stdout)
    results = payload.get("results")
    if not isinstance(results, list) or len(results) != len(public_datasets):
        raise RuntimeError("isolated benchmark worker returned an invalid result set")
    return results
