...
def scan_single_factors(
    ...
    for col in segment_columns:
        ...
        for value, subset in groups.items():
            ...
            if (
                len(ctrl) < min_subgroup_size // 2
                or len(treat) < min_subgroup_size // 2
            ):
                continue
            ...
            global_rate = (ctrl_clicks + treat_clicks) / max(
                1, ctrl_impressions + treat_impressions
            )
            ...
            candidates.append(
                {
                    "factor_id": f"{col}={value!s}",
                    "factor_name": f"{col} = {value!s}",
    ...
    for i in range(len(cols)):
        for j in range(i + 1, len(cols)):
            ...
            for (val_a, val_b), subset in groups.items():
                ...
                treat = [r for r in subset if r.get(treatment_column) == 1]
                if (
                    len(ctrl) < min_subgroup_size // 3
                    or len(treat) < min_subgroup_size // 3
                ):
                    continue
                ...
                raw_effect = (treat_clicks / max(1, treat_impressions)) - (
                    ctrl_clicks / max(1, ctrl_impressions)
                )

                candidates.append(
                    {
                        "factor_id": f"{col_a}={val_a!s}&{col_b}={val_b!s}",
                        "factor_name": f"{col_a}={val_a!s} & {col_b}={val_b!s}",
...
def scan_binned_continuous(
    ...
    for i in range(bins):
        subset = [
            r
            for r in rows
            if bin_edges[i] <= float(r.get(column, 0) or 0) < bin_edges[i + 1]
        ]
        if len(subset) < min_subgroup_size:
            continue
        ctrl = [r for r in subset if r.get(treatment_column) == 0]
        ...
        ctrl_clicks = sum(int(r.get(outcome_column, 0) or 0) for r in ctrl)
        treat_clicks = sum(int(r.get(outcome_column, 0) or 0) for r in treat)
        raw_effect = (treat_clicks / max(1, len(treat))) - (
            ctrl_clicks / max(1, len(ctrl))
        )

        candidates.append(
            {
                "factor_id": f"{column}_{_bin_label(bin_edges[i] + 0.001)}",
                "factor_name": f"{column} in {_bin_label(bin_edges[i] + 0.001)}",
...
def build_factor_report(
    ...
    # Single factor scan
    single = scan_single_factors(
        rows, treatment_column, outcome_column, segment_columns, min_subgroup_size
    )

    # Interaction scan
    interactions = scan_interactions(
        rows,
        treatment_column,
        outcome_column,
        segment_columns,
    ...
    for c in all_candidates:
        ...
        c_treat = c["treatment"]
        # Shrink both arms
        post_ctrl = posterior_summary(
            beta_posterior(c_ctrl["clicks"], c_ctrl["impressions"], global_prior)
        )
        post_treat = posterior_summary(
            beta_posterior(c_treat["clicks"], c_treat["impressions"], global_prior)
        )
    ...
    all_candidates.sort(
        key=lambda x: abs(x.get("shrunken_effect", x["raw_effect"])), reverse=True
    )
...
def _auto_detect_segments(
    ...
    for col in rows[0]:
        ...
        if col.startswith("_"):
            continue
        unique_vals = {str(r.get(col, "<missing>")) for r in rows}
...
def _group_by_two(
    rows: Sequence[Mapping[str, Any]], col_a: str, col_b: str
