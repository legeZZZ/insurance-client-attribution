...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--track2-benchmark",
        action="store_true",
        help="Run the process-isolated Track 2 hidden benchmark",
    )
    parser.add_argument(
        "--track2-benchmark-seeds",
        type=int,
        default=8,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.track2_benchmark:
        seeds = tuple(
            100 + index * 101 for index in range(max(1, args.track2_benchmark_seeds))
        )
