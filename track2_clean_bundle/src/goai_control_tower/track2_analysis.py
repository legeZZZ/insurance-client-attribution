...
def _power_screen(
    ...
    actual_per_arm = min(
        int(control.get("count", 0)), int(treatment_group.get("count", 0))
    )
    return {
...
def causal_readiness(
    ...
    design_checks = {
        ...
        "randomization_unit_matches": experiment_metadata.get("randomization_unit")
        == features.get("observation_unit"),
    }
...
def estimate_itt(
    ...
    groups = {
        arm: [row for row in rows if row.get(treatment_column) == arm] for arm in (0, 1)
    }
...
def evaluate_public_dataset(bundle: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if readiness["outcome"] == "CAUSAL_READY":
        result["estimate"] = estimate_itt(
            rows, str(experiment_metadata.get("treatment_column") or "treatment")
        )
    return result
